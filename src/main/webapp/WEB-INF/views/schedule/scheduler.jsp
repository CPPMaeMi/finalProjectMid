...
<script>
    $(function(){
    	...
    	$("#btn1").on('click',function(e){
    		...
    		function getList(data){
    			$("#sctable td").remove();
	    			...
	    			$(data).each(function(i,item){
	    				console.log(i+"번째list 다시 뿌려줌..."+item.sNum);
	    				var snum = item.sNum;
			    		var sffname = item.sffname;
						var divisionname = item.divisionname;
						var sffposition = item.sffposition;
						...
						var row = $("#sctable").append($("<tr/>").append(
							...
							$("<td><a href='javascript:void(0);' class='edit' id='updateBtn' name='"+i+"' title='Edit' >수정</a></td>"),
							$("<td><a href='javascript:void(0);' class='delete' id='delete' name='"+i+"' title='Delete'>삭제</a></td>")
							)
						);
    		...
    		$("#sctable").on('click',"#updateBtn",function(){
    			var index=$(this).prop("name");
    			var workstart=$("#workstart"+index).val();
    			var workend=$("#workend"+index).val();
    			var sNum=$("#sNum"+index).val();
    			var datec=$('#checkdate').val();
    			$.ajax({
    				url: "${cp}/schedule/updateOk.do",
  	    			type: 'post',
  	    			data: {workstart:workstart,workend:workend,sNum:sNum,workDate:datec},
  	    			contentType : "application/x-www-form-urlencoded; charset=UTF-8",
  	    			dataType : "json",
  	    			success: function(data){
  	    				console.log("성공...");
    		...
    		$("#sctable").on('click',"#delete",function(){
    			var index=$(this).prop("name");
    			var sNum=$("#sNum"+index).val();
    			var datec=$('#checkdate').val();
    			$.ajax({
    				url: "${cp}/schedule/delete.do",
  	    			type: 'get',
  	    			data: {sNum:sNum,workDate:datec},
  	    			contentType : "application/x-www-form-urlencoded; charset=UTF-8",
  	    			dataType : "json",
  	    			success: function(data){
  	    				getList(data);
